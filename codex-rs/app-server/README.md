...
- `subAgentActivity` — `{id, kind, agentThreadId, agentPath, model, reasoningEffort}` describing v2 sub-agent activity. `model` and `reasoningEffort` identify the effective child configuration on `started` events when spawn metadata is visible, and are null for other activity kinds or when spawn metadata is hidden.
